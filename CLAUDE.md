...
- **After every PR merge**: Update the plan file (`~/.claude/plans/graceful-seeking-lecun.md`), CLAUDE.md, and README.md to reflect the merged work — PR tracker, status tables, checklist items, badges
...
| `interview_coach/coach.py` | Orchestrator routing fix — explicit priority order + TOTAL_HISTORY_ENTRIES injection | ✅ |
| `interview_coach/AGENT_DESIGN_DOC.md` | Agent Design Doc — architecture, memory, decision logic, failure modes, PM decisions | ✅ |
...
- Orchestrator accuracy: initially 60% WARN — two root causes:
  - Scenario C: overlapping conditions, LLM resolved conflict incorrectly
  - Scenario E: LLM miscounted history entries by summing per-topic aggregates in summary text
- Fix: explicit priority order (P1→P2→P3, stop at first match) + `TOTAL_HISTORY_ENTRIES` as pre-computed integer
- After fix: 100% PASS (5/5 scenarios correct)

**Known debt:** System prompts in all 4 agents were generated by Claude — to be rewritten by Abhishek before Week 5.
...
- PR #22: `feature/week4-meta-evals` — orchestrator routing fix + Agent Design Doc ✅
